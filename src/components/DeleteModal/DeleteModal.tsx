...
const DeleteModal: React.FC<DeleteModalProps> = ({ term, onClose, onDelete }) => {
    ...
    const handleDelete = async () => {
        ...
        if (success) {
            showAlert(`${term} eliminado con éxito!!!`, "success");
        } else {
            showAlert(`Error al eliminar ${term}`, "error");
        }
        onClose();
    };
            ...
            <div className="bg-white p-6 rounded-md max-w-xl w-full">
                <div className="flex items-center mb-4 justify-between">
                    <div className='flex flex-row items-center'>
                        ...
                        <h2 className="text-xl font-semibold text-gray-800 ml-2">Eliminar <span>{term}</span></h2>
                    </div>
                ...
                <p className="text-gray-600 mb-4">
                    ¿Estás seguro de que deseas eliminar este <span>{term}</span>? Este será eliminado permanentemente de nuestros servidores para siempre. Esta acción no se puede deshacer.
                </p>
