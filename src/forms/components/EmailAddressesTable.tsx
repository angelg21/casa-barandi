...
interface ElectronicAddressesTableProps {
    electronicAddresses: { address: string, addressType: string }[];
    onDelete: (index: number) => void;
    onEdit: (index: number) => void;
}


export const EmailAddressesTable = ({electronicAddresses, onDelete, onEdit}: ElectronicAddressesTableProps) => {
    return (
                ...
                <div className="inline-block min-w-full align-middle">
                    <table className="min-w-full divide-y divide-gray-300 overflow-hidden rounded-lg">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                            {electronicAddresses.map((electronicAddress, index) => (
                                (electronicAddress.address != '') &&
                                <tr key={index}>
                                    <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm">
                                        <div className="font-medium text-gray-900">{electronicAddress.addressType}</div>
                                    </td>

                                    <td className="whitespace-nowrap py-4 pl-4 pr-3 text-sm">
                                        <div className="font-medium text-gray-900">{electronicAddress.address}</div>
                                    </td>
