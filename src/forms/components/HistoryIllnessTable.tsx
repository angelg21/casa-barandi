...
interface HistoryIllnessTableProps {
    historyIllness: { illnessDescription: string, dateIllness: string, severity: string }[];
    onDelete: (index: number) => void;
    onEdit: (index: number) => void;
}


export const HistoryIllnessTable = ({ historyIllness, onDelete, onEdit }: HistoryIllnessTableProps) => {
    return (
                ...
                <div className="inline-block min-w-full align-middle">
                    <table className="min-w-full divide-y divide-gray-300 overflow-hidden rounded-lg">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                            {historyIllness.map((illness, index) => (
                                (illness.illnessDescription != '') &&
                                <tr key={index}>
                                    <td className="whitespace-normal break-words py-4 pl-4 pr-3 text-sm ">
                                        <div className="font-medium text-gray-900">{illness.illnessDescription}</div>
                                    </td>

                                    <td className="whitespace-normal break-words py-4 pl-4 pr-3 text-sm">
                                        <div className="font-medium text-gray-900">{illness.severity}</div>
                                    </td>

                                    <td className="whitespace-normal break-words py-4 pl-4 pr-3 text-sm">
                                        <div className="font-medium text-gray-900">{illness.dateIllness}</div>
                                    </td>

                                    <td className="whitespace-nowrap  pl-8 pr-2 py-4 text-sm text-gray-500">
                                        <div className="flex flex-row gap-6 lg:gap-4">
                                            <button
                                                className=" text-cb-green hover:text-cb-light-green flex items-center font-medium group transition-transform transform active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
                                                onClick={() => onEdit(index)}
                                            >
                                                <svg className='mr-4' width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M8 3H3C1.89543 3 1 3.89543 1 5V16C1 17.1046 1.89543 18 3 18H14C15.1046 18 16 17.1046 16 16V11M14.5858 1.58579C15.3668 0.804738 16.6332 0.804738 17.4142 1.58579C18.1953 2.36683 18.1953 3.63316 17.4142 4.41421L8.82842 13H6L6 10.1716L14.5858 1.58579Z" stroke="#08a49c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="group-hover:stroke-cb-light-green" />
                                                </svg>
                                            </button>
