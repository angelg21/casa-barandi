...
export const SelectDateWithProps = (
    ...
    const days = [null, ...Array.from({ length: 31 }, (_, i) => 31 - i)]; // Incluye la opción "vacía"
    const months = [null, ...Array.from({ length: 12 }, (_, i) => 12 - i)]; // Incluye la opción "vacía"
    ...
    useEffect(() => {
        if (state != '') {
            if (fullDateRegex.test(state)) {
                const match = state.match(fullDateRegex);
            ...
            } else if (monthYearRegex.test(state)) {
                const match = state.match(monthYearRegex);
            ...
            } else if (yearRegex.test(state)) {
                const match = state.match(yearRegex);
