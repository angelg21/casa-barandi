...
import { deleteOrg } from '../../actions/delete-org';
...
export default function OrganizationTable({ organizations }: OrganizationTableProps) {
    ...
    const [editModalData, setEditModalData] = useState<Organizacion | undefined>(undefined);
    const router = useRouter();

    const handleClickActions = (action: string, organization: Organizacion) => {
        if (action === '01')
            router.push(`/dashboard/organizaciones/${organization.id}/detalles`);
        else if (action === '02') {
            setOpenEditModal(true);
            setEditModalData(organization)
        } else if (action === "03") {
            setIdToDelete(organization.id)
    ...
    return (
        <div className="">
                                    ...
                                    <tr key={organization.id}>
                                        <td className="whitespace-nowrap py-5 pl-4 pr-3 text-sm">
                                            <div className="flex items-center">
                                                <div className="">
                                                    <div className="font-medium text-gray-900">{organization.razon_social}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="whitespace-nowrap py-5 pl-4 pr-3 text-sm">
                                            <div className="font-medium text-gray-900">{organization.rif}</div>
                                        </td>
                                        <td className="whitespace-nowrap py-5 pl-4 pr-3 text-sm">
                                            {organization.phones.map((phone) => (
                                                <div key={phone.phoneNumber}>
                                                    <div className="mt-1 text-gray-500">{phone.phoneType}</div>
                                                    <div className="mt-1 text-gray-500">{phone.phoneNumber}</div>
                                                </div>
                                            ))}
                                        </td>
                                        <td className="whitespace-nowrap py-5 pl-4 pr-3 text-sm">
                                            {organization.electronicAddresses.map((email) => (
                                                <div key={email.address}>
                                                    <div className="mt-1 text-gray-500">{email.addressType}</div>
                                                    <div className="mt-1 text-gray-500">{email.address}</div>
                                                </div>
                                            ))}
                                        </td>
                                                    ...
                                                    {Actions.map((item) => (
                                                        <MenuItem key={item.name}>
                                                            <div
                                                                ...
                                                                onClick={() => handleClickActions(item.id, organization)}
                                                            >
            ...
            {openEditModal && (
                <OrganizationModal onClose={() => setOpenEditModal(false)} editData={editModalData} />
            )}
            {openDeleteModal && (
                <DeleteModal
                    term={"Organización"}
